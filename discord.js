...
class Music {
    ...
    async play(msg) {
        ...
        if (!this.connection[guildID]) {
            msg.channel.send('請先把機器人 `ejoin` 加入頻道哟:D');
            return;
        }

        // 如果 Bot leave 後又未加入語音頻道
        if (this.connection[guildID].status === 4) {
            msg.channel.send('請把機器人 `ejoin` 重新加入回頻道哦:3');
            return;
        }
    ...
    nowQueue(msg) {
        ...
        if (this.queue[msg.guild.id] && this.queue[msg.guild.id].length > 0) {
            ...
            msg.channel.send('先加一些歌进来再播放吧：3');
        }
...
client.on('message', async (msg) => {
    ...
    if (msg.content.indexOf(`${prefix}play`) > -1) {
        ...
        if (msg.member.voice.channel) {
            ...
            // 如果使用者不在任何一個語音頻道
            msg.reply('你要先加入机器人的語音頻道才可播放哦：D');
        }
    }
